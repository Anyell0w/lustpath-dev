...
function populateSelects() {
  const distritoSelect = document.querySelector(
    'select[name="distrito_judicial_id"]',
  );
  const organismoSelect = document.querySelector(
    'select[name="organo_jurisdiccional_id"]',
  );
  ...
  distritos.forEach((distrito) => {
    distritoSelect.innerHTML +=
      `<option value="${distrito.id_distrito}">${distrito.nombre_distrito}</option>`;
  });

  organismos.forEach((organismo) => {
    organismoSelect.innerHTML +=
...
function renderExpedientes(data) {
  ...
  tbody.innerHTML = data.map((exp) => `
    ...
    getStatusColor(exp.estado_expediente)
  }">
...
function setupEventListeners() {
  ...
  document.getElementById("searchInput").addEventListener("input", (e) => {
    ...
    const filtered = expedientes.filter((exp) =>
      ...
      (exp.nombre_distrito &&
        exp.nombre_distrito.toLowerCase().includes(searchTerm))
    );
  ...
  document.getElementById("deriveForm").addEventListener(
    "submit",
    handleDeriveSubmit,
  );
}
...
async function handleFormSubmit(e) {
  ...
  try {
    const url = currentExpediente
      ? `/api/expedientes/${currentExpediente}`
      : "/api/expedientes";
...
function renderDocuments(documents) {
  ...
  if (documents.length === 0) {
    container.innerHTML =
      '<p class="text-gray-500 text-center py-4">No hay documentos subidos</p>';
    return;
  }
            ...
            <div class="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500">
                        Subido por ${doc.usuario_subida} el ${
...
async function handleFileUpload(e) {
  ...
  try {
    const response = await fetch(
      `/api/expedientes/${currentExpediente}/documentos`,
      {
        method: "POST",
        body: formData,
      },
...
function renderUsers(users) {
  ...
  tbody.innerHTML = users.map((user) => `
    ...
    user.activo ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                <button onclick="toggleUserStatus(${user.id_usuario}, ${!user
    .activo})" 
                    class="text-indigo-600 hover:text-indigo-900 transition-colors">
...
function populateUserSelects() {
  ...
  usuarios.filter((user) =>
    user.activo && user.id_usuario !== currentUser.id_usuario
  ).forEach((user) => {
    userSelect.innerHTML +=
      `<option value="${user.id_usuario}">${user.nombre} (${user.email})</option>`;
  });
}
...
function renderDerivations(derivations) {
  ...
  tbody.innerHTML = derivations.map((der) => `
        <tr class="hover:bg-gray-50">
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                <span class="px-2 py-1 text-xs font-semibold rounded-full ${
    getDerivationStatusColor(der.estado)
  }">
                    ${der.estado}
                </span>
            </td>
...
function viewDerivationDetails(id) {
  ...
  alert(
    `Derivación: ${derivation.cod_expediente}\n\nMotivo: ${derivation.motivo}\n\nEstado: ${derivation.estado}\n\nObservaciones: ${
      derivation.observaciones || "Ninguna"
    }`,
  );
}
...
function showTab(tabName) {
  ...
  document.querySelectorAll(".tab-button").forEach((button) => {
    button.className =
      "tab-button border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm transition-colors";
  });
  ...
  document.getElementById(`${tabName}-tab`).className =
    "tab-button border-sky-500 text-sky-600 whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm";
...
async function loadDashboardStats() {
  try {
    ...
    // Update stat cards
    document.getElementById("total-expedientes").textContent =
      stats.totalExpedientes.count;
    document.getElementById("expedientes-activos").textContent =
      stats.expedientesActivos.count;
    document.getElementById("expedientes-vencidos").textContent =
      stats.expedientesVencidos.count;
    document.getElementById("expedientes-por-vencer").textContent =
      stats.expedientesPorVencer.count;
...
function renderRecentExpedientes(data) {
  ...
  if (data.length === 0) {
    container.innerHTML =
      '<p class="text-gray-500 text-sm">No hay expedientes recientes</p>';
    return;
  }
  ...
  container.innerHTML = data.map((exp) => `
    ...
    exp.sumilla ? exp.sumilla.substring(0, 40) + "..." : "Sin sumilla"
  }</p>
            </div>
...
function renderMonthlyChart(data) {
  ...
  container.innerHTML = data.map((month) => {
    ...
    const monthName = new Date(month.mes + "-01").toLocaleDateString("es-PE", {
      year: "numeric",
      month: "short",
    });
